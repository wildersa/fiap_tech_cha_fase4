...
src/
  ...
  data_loader.py              # Download de dados histórios via yfinance ou leitura de CSV.
...
- **Executar a API localmente**:
  ...
  ```
- **Dashboard Web**: Acesse `http://127.0.0.1:8000/dashboard` para interagir com o modelo, visualizar a telemetria em tempo real e consultar o Model Card.
- **Endpoints da API (Swagger / OpenAPI)**:
  ...
  *   **🔮 Inferência**
      ...
      *   `POST /predict/ohlcv`: Predição multivariada dinâmica baseada em dados OHLCV completos. Consumido no modelo de `models/lstm_petr4_multi`.
  *   **⚙️ Treinamento & MLOps (Disponíveis apenas em Modo Dev/Treinamento)**
      *   `POST /train`: Dispara síncronamente o pipeline de treinamento e auto-promove o modelo se o MAPE for melhor que o campeão.
...
flowchart LR
    ...
    B --> C[Eng. de Features: Retornos & Médias Móveis]
...
#### 2. Ciclo MLOps & Arquitetura Macro (Promoção e Endpoints)
```mermaid
flowchart LR
    subgraph Pipeline de Treinamento
        T[Novo Modelo Challenger]
    end
    
    subgraph Decisão & Governança (MLOps)
        T --> CVC{Challenger MAPE < Champion MAPE?}
        CVC -- Sim --> PM[Promoção: Atualiza models/ com ONNX & Preprocessor]
        CVC -- Não --> DM[Descarte: Mantém o Champion atual ativo]
    end
    ...
    style DM fill:#f88,stroke:#333,stroke-width:2px
```
...
Sempre que um pipeline de treinamento é concluído, o sistema compara a acurácia do novo modelo (Challenger) contra o atual em produção (Champion) utilizando o **MAPE no conjunto de Validação** (`metrics_val["mape_pct"]`).
- O Challenger só substitui o modelo em disco se o erro de validação for menor.
- O conjunto de teste (`metrics_test`) permanece estritamente isolado para validação final de generalização, eliminando vazamentos de dados de validação (*validation leakage*).
...
*   **Métrica Principal (Decisão)**
    *   **MAPE (Mean Absolute Percentage Error)**: Indicador de erro percentual médio absoluto. É a métrica soberana para a decisão do pipeline **Champion vs Challenger** (busca-se sempre o menor valor possível). É ideal para o negócio por expressar o desvio em termos relativos (%).
...
*   **Métrica de Comparação (Benchmarking)**
    *   **Ganho vs Baseline**: Medida de evolução percentual de performance do modelo treinado contra um modelo ingênuo de referência (baseline persistente, onde a previsão de $t+1$ é o valor real de $t$).
...
### Home / Card
O card exibe informações estáticas sobre o modelo em produção, como seu nome, versão, descrição, autor, fonte dos dados, métricas de performance (MAPE, MAE, RMSE, Ganho vs Baseline, Acurácia Direcional), preço de fechamento, data de fechamento e preço alvo calculado para o próximo dia.

### Inferência
O painel de inferência exibe informações dinâmicas sobre o modelo em produção, como preço de fechamento, data de fechamento e preço alvo calculado para o próximo dia.

### Treino & MLflow
O painel de treino exibe informações dinâmicas sobre o modelo em produção, como preço de fechamento, data de fechamento e preço alvo calculado para o próximo dia.
